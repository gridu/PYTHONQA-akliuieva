...
def setup_module():
    ...
    server_host = get_server_port()


def get_server_url():
    config = configparser.ConfigParser()
    config.read('configuration.ini')
    return "http://" + config['flask.server']['Host']


def get_server_port():
    config = configparser.ConfigParser()
    config.read('configuration.ini')
    return config['flask.server']['Port']
...
def test_create_book_with_valid_data_return_201():
    ...
    assert_that(response.json()["_title"], equal_to("Bridget Jones's Diary"))
    assert_that(response.json()["_book_type"], equal_to("Romance"))
    assert_that(response.json()["_creation_date"], equal_to("1996-01-12"))
...
def test_create_book_with_invalid_quantity_of_parameters_return_400():
    ...
    assert_that(response.text, equal_to("Incorrect input data"))
...
def test_create_book_with_invalid_value_of_title_return_400():
    ...
    assert_that(response.status_code, equal_to(400))
    assert_that(response.reason, equal_to("BAD REQUEST"))
    assert_that(response.text, equal_to("Incorrect input data"))
...
def test_get_book_by_existing_id_return_200():
    ...
    assert_that(response.status_code, equal_to(200))
    assert_that(response.json()["_title"], equal_to("Twilight"))
    assert_that(response.json()["_book_type"], equal_to("Drama"))
    assert_that(response.json()["_creation_date"], equal_to("2005-01-12"))
